...
SARSA is an on-policy TD control algorithm that learns the optimal policy
by updating Q-values based on the action actually taken by the current policy.

Update rule: Q(S,A) ← Q(S,A) + α[R + γQ(S',A') - Q(S,A)]

Key differences from Q-learning:
- On-policy: Learns about the policy it's following (epsilon-greedy)
- Uses Q(S',A') where A' is the action actually chosen by the policy
- More conservative than Q-learning (safer in stochastic environments)
"""
...
def sarsa_episode(env, Q, alpha, epsilon, max_steps=100):
    """
    Play one episode using SARSA.
    ...
    for step in range(max_steps):
        ...
        # Choose A' from S' using policy derived from Q (epsilon-greedy)
        next_action = select_action(env, next_state, Q=Q, epsilon=epsilon)
        ...
        if env.is_terminal(next_state):
            ...
            # Use Q(S',A') - the action we will actually take
            q_next = Q.get((next_state, next_action), 0.0)

        td_target = reward + env.discount * q_next
...
def run_sarsa(num_episodes=5000, alpha=0.1, epsilon=0.1, alpha_decay=1.0,
    ...
    for episode in range(num_episodes):
        ...
        # Run SARSA episode
        steps, reward, max_q_change = sarsa_episode(
...
if __name__ == "__main__":
    # Run SARSA
    Q, policy, rewards, q_changes = run_sarsa(
        num_episodes=5000,
        alpha=0.1,
        epsilon=0.1,
        alpha_decay=0.9999,
    ...
    print("\n" + "=" * 70)
    print("SARSA completed successfully!")
